...
async def create_chatroom(
    chatroom_data: ChatroomCreateRequest, db: Session = Depends(get_db)
) -> ChatroomResponse:
    """
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Chatroom with this name already exists"
        )
...
async def get_chatroom_users(
    chatroom_id: str, db: Session = Depends(get_db)
) -> ChatroomUserListResponse:
    """
