...
def test_data():
    ...
    return {
        ...
        "chatroom": {"name": f"smoke_test_chatroom_{str(uuid.uuid4()).replace('-', '')}"},
        "messages": [
...
def test_smoke_test_full_flow(api_client, test_data):
    ...
    user_data = api_client.create_user(
        name=test_data["user"]["name"], handle=test_data["user"]["handle"]
    )
    user_id = user_data["id"]

    # Step 3: Create chatroom
    chatroom_data = api_client.create_chatroom(name=test_data["chatroom"]["name"])
    ...
    for message_text in test_data["messages"]:
        message_data = api_client.post_message(
            message_text=message_text, user_id=user_id, chatroom_id=chatroom_id
        )
